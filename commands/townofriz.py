...
class TownOfRiz(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.games = {}

    @app_commands.command(name="town-of-riz", description="Starts Town of Riz game.")
    ...
    async def _townofriz(self, interaction: discord.Interaction, murderers: app_commands.Range[int, 1, 3]):
        ...
        embed = discord.Embed(
            title=f"{config.TOWN} Town of Riz",
            description="-# Town of Riz is a murder mystery game. Catch the murderer before he kills everyone.",
            color=0xffb900
        )
...
class TORBTN(View):
    ...
    async def _start(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed = discord.Embed(
            title=f"{config.TOWN} Town of Riz - Game Started!",
...
class ViewRoles(View):
    ...
    async def resolve_voting_phase(self, channel, game, votes):
        ...
        print(f"{alive_players} - players alive")
        print(f"{remaining_murderers} - murderers alive")    
...
async def setup(bot):
    await bot.add_cog(TownOfRiz(bot))        